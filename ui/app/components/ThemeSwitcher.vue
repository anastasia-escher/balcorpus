...
// Create a global state for our app if it doesn't exist
const appState = useState('appState', () => ({
  theme: 'Aura', // default theme can be adjusted here
}))

const presetMap = {Aura, Lara, Nora}
...
// Use the global app state for theme binding
const theme = computed({
  get: () => appState.value.theme,
  set: value => (appState.value.theme = value),
})

const rippleActive = computed(() => nuxtApp.$primevue.config.ripple)
...
// Generate the preset extension configuration based on the selected primary color
const getPresetExt = () => {
...
// Update the colors and apply the theme based on the type
...
// Apply theme changes based on the update
...
// Handle preset changes from the SelectButton
const onPresetChange = value => {
  appState.value.theme = value
 ...
 *  NEW PART:
 * -----------
 * Create a computed property that merges the user’s current selections
 * into a “copy & paste” code snippet.
 */
const codeSnippet = computed(() => {
  const chosenPreset = presetMap[theme.value]
  const presetExtension = getPresetExt()
  const chosenSurface = surfaces.find(s => s.name === selectedSurfaceColor.value)?.palette

  return `// Copy/Paste this config into a Nuxt plugin file, e.g. plugins/primeuix.ts
import { $t } from '@primeuix/themes'
import Aura from '@primeuix/themes/aura'
import Lara from '@primeuix/themes/lara'
import Nora from '@primeuix/themes/nora'

// If you need these arrays, you can also copy from your code:
// const primaryColors = [...]
// const surfaces = [...]
...
export default defineNuxtPlugin(() => {
  $t()
    // Base preset
    .preset(${JSON.stringify(chosenPreset, null, 2)})

    // Extension for the selected primary color
    .preset(${JSON.stringify(presetExtension, null, 2)})

    // Override surface palette
    .surfacePalette(${JSON.stringify(chosenSurface, null, 2)})
...
<template>
  <div class="card flex justify-end p-2 mb-4">
    <ul class="flex list-none m-0 p-0 gap-2 items-center">
      ...
      <li class="relative">
        ...
        <div
          class="absolute top-[2.5rem] right-0 hidden w-[16rem] p-3 bg-white dark:bg-blue-800 rounded-md shadow border border-blue-200 dark:border-blue-700 flex-col justify-start items-start gap-3.5 inline-flex origin-top z-10">
          <div class="flex-col justify-start items-start gap-2 inline-flex pr-4 just">
            ...
            <div class="self-stretch justify-start items-start gap-2 inline-flex flex-wrap">
              <button
                ...
                :style="{
                  backgroundColor: primaryColor.name === 'noir' ? 'black' : primaryColor.palette['500'],
  ...
  <div class="card p-3 mb-4">
    <h4 class="mb-2 font-medium">Copy & Paste Configuration</h4>
    <pre class="text-xs bg-gray-100 dark:bg-gray-800 p-3 rounded whitespace-pre-wrap"
      >{{ codeSnippet }}
    </pre>
  </div>
</template>
